...
def coupling_function(
    ...
    # INITIALIZE OUTPUT ARRAYS - DEFAULT VALUES ARE '0' AND 'NO DATA'
    ...
    cf = CoupFunc(
            name, freqs, factors, flags,
            sens_q, sens_inj, darm_q, darm_inj,
            time_q, time_inj, calibration_factors
        )    
    return cf
...
def bin_coupling_data(data, width):
    ...
    data_binned = CoupFunc(
        data.name, freqs_binned, factors_binned, flags_binned,
        sens_bg_binned, data.sens_inj, darm_bg_binned, data.darm_inj,
        data.t_bg, data.t_inj, data.calibration_factors
    )
    return data_binned



def coupling_function_binned(df, width):
    """
    Logarithmic binning of coupling function data in pandas dataframe format
...
def composite_coupling_function(cf_data_list, injection_names, local_max_window=0, freq_lines=None):
    ...
    if len(cf_data_list) == 1:
        ...
        return comp_data
#         return cf.factors, cf.factors_in_counts, cf.flags, np.asarray(injection_cols[0])
    ...
    comp_cf = CompositeCoupFunc(
        channel_name, freqs, factors, factors_counts, flags, injs, sens_bg, darm_bg
    )
    return comp_cf
#     return factors, factors_counts, flags, injs
...
def smooth_comp_data(data, width):
    ...
    smooth_data = couplingfunction.CompositeCouplingData(
